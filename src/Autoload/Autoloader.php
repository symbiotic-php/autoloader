...
class Autoloader
{
        ...
        if ($file) {
            ...
            \file_put_contents(
                ...
                '<?php ' . PHP_EOL . 'return ' . \var_export([
                                                                 'namespaces' => static::$registered_namespaces,
    ...
    private static function loadFiles(array $files): void
    {
    ...
    protected static function loadPackages(string $dir)
    {
        ...
        foreach ($iterator as $fileInfo) {
            ...
            if (\is_readable($composer_file)) {
                $loader = self::getComposerLoader(
                    $fileInfo->getRealPath() . '/composer.json',
                    ...
                    if (!empty($loader['namespaces'])) {
                        foreach ($loader['namespaces'] as $namespace => $data) {
                            ...
                            if (array_key_exists($namespace, self::$registered_namespaces)) {
                                $data['root_dir'] = array_merge(
                                    self::$registered_namespaces[$namespace]['root_dir'],
                                    $data['root_dir']
                                );
                            }
                    ...
                    if (isset($autoload['psr-4']) && \is_array($autoload['psr-4'])) {
                        foreach ($autoload['psr-4'] as $namespace => $dir) {
                            ...
                            foreach ((array)$dir as $v) {
                                $dirs [] = $base_dir . DIRECTORY_SEPARATOR . trim($v, '\\/');
                            }
